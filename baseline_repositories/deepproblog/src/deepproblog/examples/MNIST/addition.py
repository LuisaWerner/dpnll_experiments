...
# seeds =
test_accuracies, symbolic_iter_times, total_times = [], [], []
...
for i in N_values:
    for n, _seed in enumerate(range(len(seeds))):
        ...
        run_total_time = time.time() - total_start
        total_times.append(run_total_time)
        print(f" total time: {run_total_time}")
    ...
    for j, accuracy in enumerate(test_accuracies):
        result_str += f"Run {j}: Test Accuracy: {accuracy:.4f}, Total runtime: {total_times[j]}\n"
    ...
    mean_total_runtime = np.mean(total_times)
    ...
    result_str += f"Mean total runtime : {mean_total_runtime:.6f}s\n"

    # Print the result string
    print(result_str)

    # Write the result string to a file
    with open("log/" + f"{method}/" + f"{i}Sum/" + "summary_2.txt", 'w') as file:
